...
seasons = queries.get_seasons(conn, league).iloc[:, 0].tolist()
index = seasons.index("22-23") if "22-23" in seasons else 0
...
teams = queries.get_teams_from_season(conn, country, league, season).iloc[:, 0].tolist()
index1 = teams.index("Liverpool") if "Liverpool" in teams else 0
home_team = st.sidebar.selectbox(
    "Home Team",
    teams,
    index=index1)

index2 = teams.index("Man United") if "Man United" in teams else 0
...
if match_df.empty:
    st.info("No match statistics found for the selected filters.")
    st.stop()
